...
export const getDecimalRoundedTo2 = (num: number) =>
 ...
 */
export const getNumberRoundedTo2 = (num: number) =>
...
const shortenLargeNumberWithUnit = (number: number) => {
  ...
  const tier =
    (log10(abs(number)) / 3) | 0; /* eslint-disable-line no-bitwise */
...
export const simpleCurrencyDisplay = (
 ...
 * ```
 */
export const getHumanReadableBytes = (
...
export const formatDurationForVideoDisplay = (duration: number | undefined) => {
  ...
  return `${h ? `${h}:` : ""}${h ? String(m).padStart(2, "0") : m}:${String(
    s
  ).padStart(2, "0")}`;
};

export const formatTimeForTimerDisplay = (
  s: number,
  version: "sec" | "min" | "full" | "dynamic"
) => {
  if (s <= 0) {
...
export const objectByKey = <
  ...
  T extends Object & { [Key in K]: string | number }
>(
  arr: T[],
  key: K
) =>
  arr.reduce(
    (prev, obj) => ({ ...prev, [obj[key]]: obj }),
    {} as Record<T[K], T>
  );
...
>(
  ...
  arr.reduce(
    (prev, obj) => ({ ...prev, [obj[primaryKey][secondaryKey]]: obj }),
    {} as Record<T[PK][SK], T>
  );
 ...
 * ```
 */
export const objectOfArraysByKey = <
  K extends string | number,
  T extends Object & { [Key in K]: string | number }
>(
  arr: T[],
...
export const permitKeys = <T extends object>(keys: (keyof T)[], obj: T) =>
  keys.reduce(
    (prev, curr) => (obj[curr] ? { ...prev, [curr]: obj[curr] } : prev),
    {} as T
  );

type SimpleParamValueType = number | string | string[] | boolean | null;
type ParamValueType =
  | SimpleParamValueType
  | Record<string, SimpleParamValueType>
  | undefined;
/**
...
export const buildQueryParamsString = (
  queryParams:
    ...
    | { name: string; value: ParamValueType }[],
  prefixKey?: string
) => {
  ...
  if (!prefixKey && Array.isArray(queryParams)) {
    stringifiedQueryParams = queryParams
      .map(
        (paramObj) =>
          `${encodeURIComponent(paramObj.name)}=${encodeURIComponent(
            String(paramObj.value)
          )}`
      )
      .join("&");
    ...
    stringifiedQueryParams = Object.keys(queryParams)
      ...
      .map((key) => {
        ...
        if (!prefixKey && typeof value === "object") {
          return buildQueryParamsString(
            value as Record<string, ParamValueType>,
            key
          );
        }
          ...
          if (value && Array.isArray(value)) {
            return value.reduce(
              (prev, val, i) =>
                `${
                  prev ? `${prev}&` : ""
                }${`${prefixKey}[${key}][${i}]=${encodeURIComponent(val)}`}`,
              ""
            );
          }
  ...
  return stringifiedQueryParams
    ? `${prefixKey ? "" : "?"}${stringifiedQueryParams}`
    : "";
};
...
export const generateAddToGoogleCalendarUrl = (
  ...
  `https://www.google.com/calendar/render${buildQueryParamsString({
    ...
    dates: [
      ...
      sessionDetails.startsAt + minutes(sessionDetails.durationInMinutes).ms,
    ]
    ...
    recur:
      recur &&
      `RRULE:${(
        Object.entries(recur.recurPattern) as [
          keyof RecurPattern,
          RecurPattern[keyof RecurPattern]
        ][]
      ).reduce(
        (prev, [key, val]) =>
          `${prev}${key}=${Array.isArray(val) ? val.join(",") : val};`,
        ""
      )}`,
  })}`;
...
export function validatePhoneNumber(str: string) {
  ...
  if (
    ...
    (onlyNumbers.length === 11 && onlyNumbers[0] !== "1")
  ) {
