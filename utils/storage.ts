...
export type CustomLocalStorage = {
  Version: number;
  GeoLocation: { longitude: number; latitude: number } | null;
};

const parseValue = <T extends keyof CustomLocalStorage>(
  value: string | null,
  fallback: CustomLocalStorage[T] | null
) => {
  if (value === null) {
    return fallback;
  }
 ...
 */
export const setLocal = <T extends keyof CustomLocalStorage>(
  key: T,
  data: CustomLocalStorage[T]
...
export const getLocal = <T extends keyof CustomLocalStorage>(
  key: T,
  fallback: CustomLocalStorage[T] | null = null
) => {
 ...
 * @param key The local storage key
 */
export const removeLocal = (key: keyof CustomLocalStorage) =>
...
export const addSetLocalEventListener = (
  key: keyof CustomLocalStorage,
  cb: VoidFunction
) => {
  document.addEventListener(`${SET_LOCAL_EVENT_PREFIX}${key}`, cb);
  return () =>
    document.removeEventListener(`${SET_LOCAL_EVENT_PREFIX}${key}`, cb);
};
...
export const addRemoveLocalEventListener = (
  key: keyof CustomLocalStorage,
  cb: VoidFunction
) => {
  document.addEventListener(`${REMOVE_LOCAL_EVENT_PREFIX}${key}`, cb);
  return () =>
    document.removeEventListener(`${REMOVE_LOCAL_EVENT_PREFIX}${key}`, cb);
};
