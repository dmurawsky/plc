...
export const milliseconds = (numUnits: N) =>
  getConverter(numUnits, "milliseconds");
...
export const getMonthStart = (d: Date) =>
  new Date(d.setDate(1)).setHours(0, 0, 0, 0);

export const getBillingPeriodTimeFromSubscription = (
  period: Period,
  interval: number
) => {
  switch (period) {
...
export const getDisplayTimeWithTimezone = (
  timestamp: number,
  locale = "default",
  timeZone?: string
) => {
...
export const getDisplayDateWithTimezone = (
  timestamp: number,
  locale = "default",
  timeZone?: string
) => {
...
export const convertNativeDateInputToMs = (value: string | undefined) => {
...
export const getDateTimeForEmail = (
  ...
  const time = getDisplayTimeWithTimezone(timestamp, locale, timeZone).split(
    "-"
  )[0];
...
export const getSundayAtMidnightUTC = (
  today: boolean = false,
...
export const setMilitaryTimeOnDate = (
  militaryTime: number,
  startsAt: number
) => {
...
export const getStartTimePlusDuration = (
  startTime: number,
  durationInMinutesMultiple: number
) => {
  const startTimeInHours = convertMilitaryTimeToHours(startTime);
  const startTimeInMinutes = hours(startTimeInHours).in("minutes");
  const durationInHoursMultiple = minutes(durationInMinutesMultiple).in(
    "hours"
  );

  const hrs = Math.floor(startTimeInHours + durationInHoursMultiple) * 100;
  const min =
    (startTimeInMinutes + durationInMinutesMultiple) % hours(1).in("minutes");

  return Math.round(hrs + min);
};

export const getMinutesBetweenMilitaryTimes = (
  startTime: number,
  endTime: number
) => {
...
export const getStartAndEndMilitaryTimes = (
  startsAt: number,
...
export const moveDate = (d: Date, move: number) =>
  new Date(d).setDate(d.getDate() + move);
 ...
 */
export const getGmtDiff = (
  timeZone: (typeof IANNA_TIME_ZONES)[number]["key"]
) => {
  const date = new Date();
  ...
  const hrsAccountingMinutes =
    minDiff < 0 ? hrs + minutes(minDiff).in("hours") : hrs;

  // NOTE: Remember that adding a negative number subtracts
  const finalHours = String(
    Math.abs(Math.floor(hrsAccountingMinutes))
  ).padStart(2, "0");
  const finalMinutes = String(minDiff < 0 ? 60 + minDiff : minDiff).padStart(
    2,
    "0"
  );
 ...
 */
export const setStartTimeOnStartsAt = (
  startsAt: string,
  startTime: number,
  timeZone: string
) => {
  ...
  const timeZoneDate = new Date(
    new Date(startsAt).toLocaleString("en-US", { timeZone })
  );
  ...
  return new Date(startsAt)
    .setHours(militaryHoursAfterTimeZone, militaryMinutesAfterTimeZone, 0, 0)
    .valueOf();
};
...
const getTimeDifferenceString = (
  ...
  return `${past ? "" : "in "}${rounded} ${unit}${rounded === 1 ? "" : "s"}${
    past && includeAgo ? " ago" : ""
  }`;
};
...
export const timeDifference = (
  ...
  if (diff < ONE_DAY_IN_MS)
    return getTimeDifferenceString(diff / ONE_HOUR_IN_MS, "hour", includeAgo);
  if (diff < ONE_MONTH_IN_MS)
    return getTimeDifferenceString(diff / ONE_DAY_IN_MS, "day", includeAgo);
  if (diff < ONE_YEAR_IN_MS)
    return getTimeDifferenceString(diff / ONE_MONTH_IN_MS, "month", includeAgo);
